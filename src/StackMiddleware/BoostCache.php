...
namespace Drupal\boost\StackMiddleware;
 ...
 */
class BoostCache implements HttpKernelInterface {
   ...
   *   A policy rule determining the cacheability of the response.
   */
  public function __construct(HttpKernelInterface $http_kernel, CacheBackendInterface $cache, RequestPolicyInterface $request_policy, ResponsePolicyInterface $response_policy, BoostCacheFile $cacheFile, AttachmentsResponseProcessorInterface $html_response_attachments_processor) {
  ...
  protected function fetch(Request $request, $type = self::MASTER_REQUEST, $catch = TRUE) {
    ...
    if ($this->storeResponse($request, $response)) {
      $response->headers->set('X-Boost-Cache', 'MISS');
    }
  ...
  protected function storeResponse(Request $request, Response $response) {
    ...
    if ($response instanceof RedirectResponse || $response instanceof TrustedRedirectResponse) {
      return FALSE;
    }
    ...
    \Drupal::moduleHandler()->alter('boost_cache_tags', $tags,$request);

    if(in_array('none_boost_cache', $tags)){
      return FALSE;
    }
  ...
  protected function get(Request $request, $allow_invalid = FALSE) {
    ...
    if ($cache = $this->cache->get($cid, $allow_invalid)) {
      $fileUri = $cache->data;
      $content = $this->cacheFile->load($fileUri);
      if($content){
  ...
  protected function set(Request $request, Response $response, $expire, array $tags) {
    if($response->headers->get('X-Drupal-Cache') !=  'HIT') {
        ...
        $fileUri = $this->getCacheFileUri($request);
    ...
    ";
        ...
        $this->cacheFile->save($fileUri, $content . $boost_suffix);
        $this->cache->set($cid, $fileUri, $expire, $tags);
    }
  }
   ...
   *   The cache ID for this request.
   */
  protected function getCacheFileUri(Request $request) {
    $requestUri = $request->getRequestUri();
    $base_path = base_path();
    if($requestUri == $base_path){
      $requestUri = $base_path . 'index';
    }
    $context_prefix = $request->headers->get('CONTEXT_PREFIX');
    if(!empty($context_prefix)){
      $requestUri = substr($requestUri, strlen($context_prefix));
    }
    $uri = file_default_scheme() . '://boost/' . $request->getHost() . $requestUri . '.' . $request->getRequestFormat();
    return $uri;
  }
